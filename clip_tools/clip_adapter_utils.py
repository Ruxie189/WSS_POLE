...
class Adapter(nn.Module):
    def __init__(self, c_in, reduction=4):
        ...
        self.ratio_vector = nn.Parameter(0.2 * torch.ones(512))
...
def get_best_prompts_from_json():
    with open('/home/ruxie/scratch/ruxie/GPT_v2_4.json','rb') as f:
        data = pickle.load(f)
    return data
...
import clip
def clip_forward(clip_model, Av, At, images, labels, dname='coco',sen='key',fnames=None):
    #print(labels)
    texts = to_text(labels, dname)
    #texts_new = get_texts_for_image_ab(texts,sen)
    #print(texts_new)
    ...
    #Custom Clip forward:
    ###################################################
    #print(images.shape, foreground_texts.shape)
    ...
    image_features = Av.ratio_vector * xi + (1 - Av.ratio_vector) * image_features
    text_features = At.ratio_vector * xt + (1 - At.ratio_vector) * text_features
