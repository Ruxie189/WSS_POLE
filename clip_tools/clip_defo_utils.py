...
class PromptLearner(nn.Module):
    def __init__(self, clip_model, device):
        ...
        #initiate class context dicts
        classnames = category_dict["voc"]
        classnames = [name.replace("_", " ") for name in classnames]
        prompts = [name for name in classnames]
        context_dict = {}
        for i, prompt in enumerate(prompts):
            tokenized_prompt = clip.tokenize(prompt).to(device)
            with torch.no_grad(): embedding = clip_model.token_embedding(tokenized_prompt).type(clip_model.dtype)
            ctx_vectors_2 = embedding[0, 1 : 2, :]
            if classnames[i] == "train": context_dict["trn"] = nn.Parameter(ctx_vectors_2)
            else: context_dict[classnames[i]] = nn.Parameter(ctx_vectors_2)
        print(context_dict.keys())
        self.cls = nn.ParameterDict(context_dict)
    ...
    def update(self, classnames, clip_model, device):
        ...
        self.register_buffer("token_suffix", embedding[:, 2 + self.n_ctx :, :])  # CLS, EOS
        self.batch_cls = classnames
    ...
    def forward(self):
        ...
        ctx2_list = []
        for name in self.batch_cls:
            if name != "train":
                ctx2_list.append(self.cls[name].unsqueeze(0))
            else: ctx2_list.append(self.cls["trn"].unsqueeze(0))
        ctx2 = torch.cat(ctx2_list, dim=0)
        ...
        prompts = torch.cat(
            [
                ...
                ctx2.to(self.device),    # (n_cls, 1, dim)
...
def clip_forward(clip_model, prompt_learner, device, images, labels, dname='coco',sen='key', fnames=None):
    ...
    #DeFo Custom Clip Init stuff goes here:
    ###################################################
