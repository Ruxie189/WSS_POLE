...
class Net(nn.Module):
    ...
    def __init__(self, stride=16, n_classes=20):
        ...
        self.image_adapter = Adapter(2048)
        self.text_adapter = Textadapter(512)
        self.classifier = nn.Conv2d(2048, n_classes, 1, bias=False)

        self.backbone = nn.ModuleList([self.stage1, self.stage2, self.stage3, self.stage4])
        self.newly_added = nn.ModuleList([self.classifier])
        self.adapter = nn.ModuleList([self.image_adapter, self.text_adapter])
    ...
    def forward(self, x):
        ...
        x = self.image_adapter(x)  # added adapter for no reason :)
        x = self.classifier(x)
    ...
    def trainable_parameters(self):
        ...
        return (list(self.backbone.parameters()), list(self.newly_added.parameters()), list(self.adapter.parameters()))
