...
from clip_tools.clip_utils import clip_forward
...
def run(args):
    if args.clims_network == "net.resnet50_clims": 
    ...
    elif args.clims_network == "net.resnet50_clims_adapt": 
        model = getattr(importlib.import_module(args.clims_network), 'CLIMS')(n_classes=20)
        model.load_state_dict(torch.load(args.cam_weights_name, map_location = 'cpu'), strict=True)
        #model.load_state_dict(torch.load('cam-baseline-voc12/res50_cam.pth'), strict=True)
    else:
        model = getattr(importlib.import_module(args.clims_network), 'CLIMS')(patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
    ...
    #model.load_state_dict(torch.load('cam-baseline-voc12/resnet50-11ad3fa6.pth'), strict=True)
    model.load_state_dict(torch.load(args.cam_weights_name), strict=True)
    ...
    if args.clims_network == "net.resnet50_clims": 
        ...
        optimizer = torchutils.PolyOptimizer([
            {'params': param_groups[0], 'lr': args.clims_learning_rate, 'weight_decay': args.cam_weight_decay},
            {'params': param_groups[1], 'lr': 10 * args.clims_learning_rate, 'weight_decay': args.cam_weight_decay},
            {'params': param_groups[2], 'lr': args.clims_learning_rate, 'weight_decay': args.cam_weight_decay},
        ], lr=args.clims_learning_rate, weight_decay=args.cam_weight_decay, max_step=max_step)
    else:
        param_groups = model.trainable_parameters()
        optimizer = torchutils.PolyOptimizer([
    ...
    for ep in range(args.clims_num_epoches):
        ...
        for step, pack in enumerate(train_data_loader):
            ...
            if not args.clims_use_adapter:
                L_OTM = OTMLoss(clip_forward(clip_model, fg_224_eval, fg_label[fg_indices], dname='voc',sen=args.clims_texts,fnames=fnames, file= args.json_file), 1)

                L_BTM = BTMLoss(clip_forward(clip_model, bg_224_eval, fg_label[fg_indices], dname='voc',sen=args.clims_texts,fnames=fnames, file=args.json_file), 1)
            else:
                L_OTM = OTMLoss(clip_forward(clip_model, fg_224_eval, fg_label[fg_indices], dname='voc',sen=args.clims_texts,fnames=fnames, file=args.json_file, adapter=model.module.text_adapter), 1)

                L_BTM = BTMLoss(clip_forward(clip_model, bg_224_eval, fg_label[fg_indices], dname='voc',sen=args.clims_texts,fnames=fnames, file=args.json_file, adapter=model.module.text_adapter), 1)
            L_CBS = CBSLoss(clip_model, fg_224_eval,fg_label[fg_indices], fnames=fnames)
