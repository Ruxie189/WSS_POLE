...
from clip_tools.clip_adapter_utils import Adapter, clip_forward
...
def run(args):
    ...
    model.Av, model.At = Adapter(512), Adapter(512)
    ...
    param_groups = (list(model.backbone.parameters()), list(model.newly_added.parameters()), list(model.adapters.parameters()))
    ...
    optimizer = torchutils.PolyOptimizer([
        ...
        {'params': param_groups[2], 'lr': args.clims_adapter_lr_factor * args.clims_learning_rate, 'weight_decay': args.cam_weight_decay},
    ...
    for ep in range(args.clims_num_epoches):
        ...
        for step, pack in enumerate(train_data_loader):
            ...
            L_OTM = OTMLoss(clip_forward(clip_model, model.module.Av, model.module.At, fg_224_eval, fg_label[fg_indices], dname='voc',sen=args.clims_texts,fnames=fnames), 1)

            L_BTM = BTMLoss(clip_forward(clip_model, model.module.Av, model.module.At, bg_224_eval, fg_label[fg_indices], dname='voc',sen=args.clims_texts,fnames=fnames), 1)
